  ...
  (async () => {
    if (this.options.enableGlobalSetup) {
      ...
      try {
        ...
        debug('caught error when running global setup', err);
        done(err);
        return;
      }
      runner.run(
        async () => {
          if (this.options.enableGlobalSetup) {
            ...
            try {
              ...
              debug('caught error when running global teardown', err);
              done(err);
            }
          }
        },
        {files: this.files, options}
      );
    } else {
      runner.run(done, {files: this.files, options});
    }
  })();
  return runner;
};
 ...
 * @chainable
 * @public
 * @param {MochaGlobalFixture|MochaGlobalFixtures[]} [setupFns] - Global setup fixture(s)
 * @returns {Mocha}
 */
 ...
 * @chainable
 * @public
 * @param {MochaGlobalFixture|MochaGlobalFixtures[]} [teardownFns] - Global teardown fixture(s)
 * @returns {Mocha}
 */
...
/**
 * Run any global setup fixtures sequentially.
 *
 * This is called by {@link Mocha#run} _unless_ the `runGlobalSetup` option is `false`.
 *
 * The context object this function resolves with should be consumed by
 * {@link Mocha#runGlobalTeardown}.
...
/**
 * Run any global teardown fixtures sequentially.
 *
 * This is called by {@link Mocha#run} _unless_ the `runGlobalTeardown` option is `false`.
 *
 * SHould be called with `context` returned by {@link Mocha#runGlobalSetup}.
 * @param {object} [context] - Context object if already have one
 * @private
 * @returns {Promise<object>} Context object
 */
